...
def save_checkpoint(step_name, model=None, metadata=None):
    ...
    config_data = {
        ...
        "timestamp": timestamp,
        "step_name": step_name,
        "instructions": "To set up the Phi4COCONUT model for inference, load the state_dict from this checkpoint file into your model and use the provided configuration parameters.",
        "model_architecture": {
            "model_type": "Phi4COCONUTModel",
            "components": {
...
class Phi4COCONUTModel(nn.Module):
    """
    ...
    This version also incorporates a diffusion-based LLM approach (inspired by Mercury)
    for efficient parallel text generation.
    """
    ...
    def __init__(
        ...
        """
        Initialize the Phi4COCONUTModel.
        ...
        """
        super(Phi4COCONUTModel, self).__init__()
    ...
    def forward(self, input_ids, attention_mask, labels, position_ids, **kwargs):
        """
        Forward pass through the model with TOVA compression.
        ...
        if use_diffusion and self.use_diffusion:
            return self.forward_diffusion(input_ids, attention_mask, labels, position_ids, **kwargs)
        else:
            return self.forward_autoregressive(input_ids, attention_mask, labels, position_ids, **kwargs)
    
    def forward_autoregressive(self, input_ids, attention_mask, labels, position_ids, **kwargs):
        """
        Forward pass using the autoregressive approach with COCONUT and TOVA.
            ...
            if kv_cache is None:
                ...
                if self.use_tova and len(kv_cache) > 0 and attention_weights:
                    ...
                    if token_importance is not None:
                        ...
                        # Calculate token information content from hidden states
                        last_hidden = outputs.hidden_states[-1]
                        
                        # Advanced information content estimation using multiple features
                        # 1. Activation variance (traditional entropy estimation)
                        variance_score = torch.var(last_hidden, dim=-1).mean(dim=0)
                        
                        # 2. Gradient of activations (rate of change indicates importance)
                        if last_hidden.size(1) > 1:
                            gradient_score = torch.abs(last_hidden[:, 1:] - last_hidden[:, :-1]).mean(dim=(0, 2))
                            # Pad to match original size
                            gradient_score = F.pad(gradient_score, (0, 1), value=gradient_score.mean().item())
                        else:
                            gradient_score = torch.ones_like(variance_score)
                        
                        # 3. Activation magnitude (larger activations may indicate importance)
                        magnitude_score = torch.norm(last_hidden, dim=2).mean(dim=0)
                        
                        # Combine scores (weighted sum)
                        token_info = 0.4 * variance_score + 0.3 * gradient_score + 0.3 * magnitude_score
                        
                        # Normalize to [0, 1] range
                        if token_info.max() > token_info.min():
                            token_info = (token_info - token_info.min()) / (token_info.max() - token_info.min())
                        
                        compressed_keys, compressed_values = self.tova_compressor.compress_with_entropy(
                            attention_weights[-1],  # Use last layer attention weights
                            torch.stack(keys),
                            torch.stack(values),
                            token_info
                    ...
                    # Track compression statistics for learning
                    self.compression_stats["total_compressions"] += 1
                    self.compression_stats["tokens_kept_ratio"].append(
                        self.tova_cache_size / len(kv_cache[0][0][0])
                    )
                    
                    # Calculate attention pattern similarity before and after compression
                    if len(attention_weights) > 0 and attention_weights[-1].size(1) > 0:
                        # Compute attention pattern on original vs compressed keys
                        orig_pattern = F.softmax(torch.matmul(
                            attention_weights[-1][:, -1:],
                            keys[0].transpose(-2, -1)
                        ) / (keys[0].size(-1) ** 0.5), dim=-1)
                        
                        comp_pattern = F.softmax(torch.matmul(
                            attention_weights[-1][:, -1:],
                            compressed_keys[0].transpose(-2, -1)
                        ) / (compressed_keys[0].size(-1) ** 0.5), dim=-1)
                        
                        # Calculate cosine similarity between patterns
                        similarity = F.cosine_similarity(
                            orig_pattern.reshape(-1),
                            comp_pattern.reshape(-1),
                            dim=0
                        ).item()
                        
                        self.compression_stats["attention_pattern_similarity"].append(similarity)
                    
                    # Reconstruct kv_cache with compressed keys and values
                    kv_cache = [(k, v) for k, v in zip(compressed_keys, compressed_values)]
                    
                    # Update TOVA compressor based on compression performance
                    if len(self.compression_stats["attention_pattern_similarity"]) > 5:
                        avg_similarity = sum(self.compression_stats["attention_pattern_similarity"][-5:]) / 5
                        # If similarity is too low, adjust learning rate or entropy weight
                        if avg_similarity < 0.7:
                            # Increase entropy weight to preserve more important tokens
                            self.tova_compressor.entropy_weight = min(0.8, self.tova_compressor.entropy_weight + 0.05)
                        elif avg_similarity > 0.95:
                            # Decrease entropy weight if compression is too conservative
                            self.tova_compressor.entropy_weight = max(0.1, self.tova_compressor.entropy_weight - 0.05)
        ...
        if self.use_tova and kv_cache is not None and attention_weights:
            ...
            # Update compression statistics for learning
            if hasattr(self.tova_compressor, 'get_stats'):
                compressor_stats = self.tova_compressor.get_stats()
                if 'average_compression_ratio' in compressor_stats:
                    self.compression_stats["compression_efficiency"].append(
                        compressor_stats['average_compression_ratio']
                    )
    ...
    def forward_diffusion(self, input_ids, attention_mask, labels, position_ids, **kwargs):
        """
        Forward pass using the diffusion-based approach.
        ...
        Returns:
            Outputs namedtuple with loss, inputs_embeds, and logits
        """
        # Get embeddings from the joint embedding layer
        inputs_embeds = self.joint_embedding(input_ids)
        
        # Forward pass through diffusion LLM
        diffusion_outputs = self.diffusion_llm(tokens=input_ids)
        ...
        if labels is not None:
            ...
            combined_loss = 0.5 * diffusion_loss + 0.5 * ce_loss
        else:
    ...
    def generate(
        ...
        **kwargs
        ...
        """
        Generate text using the model with TOVA compression.
        ...
        if use_diffusion_gen:
            return self.generate_diffusion(
                input_ids,
                attention_mask=attention_mask,
    ...
    def generate_diffusion( self,
        ...
        **kwargs
        ...
        """
        Generate text using the diffusion-based approach.
        ...
        prompt_embeddings = self.joint_embedding(input_ids)
        
        # Determine maximum sequence length
        max_seq_len = input_ids.shape[1] + max_new_tokens
        
        # Generate with ODE solver or step-by-step approach
        if use_ode:
            # Generate using ODE solver (faster)
            generated_tokens = self.diffusion_llm.generate_with_ode(
            ...
            generated_tokens = self.diffusion_llm.generate_parallel(
                prompt_embeddings=prompt_embeddings,
                max_seq_len=max_seq_len,
                temperature=temperature
            )
    ...
    def generate_text(self, prompt, max_new_tokens=1000, images=None, audios=None, videos=None, use_diffusion=None,
        ...
        # Generate with diffusion or autoregressive approach
        if use_diffusion_gen:
    ...
    def apply_length_penalty_to_reward(
        self,
        reward: float,
        sequence_length: int,
        ...
        """
        Apply length penalty to reinforcement learning reward.
        ...
        """
        if not hasattr(self, 'tova_compressor') or not self.use_tova:
            # If TOVA is not enabled, implement penalty directly
            ...
            # Apply penalty to reward, preserving accuracy component if provided
            if accuracy_component is not None:
                non_accuracy = reward - accuracy_component
                return accuracy_component + max(0, non_accuracy - penalty)
            else:
                return max(0, reward - penalty)
        else:
            # Use TOVA compressor's penalty method if available
            return self.tova_compressor.modify_rl_reward(
                reward=reward,
    ...
    def train_with_continual_propagation(self, dataloader, optimizer, criterion, num_epochs=1,
        ...
        print("Playing sound to notify that model training is complete...")
    ...
    def save_pretrained(self, output_dir):
        ...
        with open(os.path.join(output_dir, "phi4coconut_config.json"), "w") as f:
    ...
    def from_pretrained(cls, model_path, **kwargs):
        ...
        # Load configuration
        config_path = os.path.join(model_path, "phi4coconut_config.json")
...
if __name__ == "__main__":
    # Create the Phi4COCONUTModel
    model = Phi4COCONUTModel()
